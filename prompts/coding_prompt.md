...
**CRITICAL: Execute tools ONE AT A TIME, sequentially. NEVER make parallel/concurrent tool calls - this causes API errors. Wait for each tool to complete before calling the next.**

### STEP 1: GET YOUR BEARINGS (MANDATORY)

Start by orienting yourself. Run this SINGLE combined command:

```bash
# Get all context in ONE command (DO NOT split into multiple parallel calls)
