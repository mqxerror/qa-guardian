...
function parseUptimeCheckRow(row: UptimeCheckRow): UptimeCheck {
  return {
    ...
    locations: safeJsonParseOrPassthrough(row.locations, []) as UptimeCheck['locations'],
    assertions: safeJsonParseOrPassthrough(row.assertions, []) as UptimeCheck['assertions'],
...
function parseCheckResultRow(row: CheckResultRow): CheckResult {
  return {
    ...
    assertion_results: safeJsonParseOrPassthrough(row.assertion_results, []) as CheckResult['assertion_results'],
    assertions_passed: row.assertions_passed,
    assertions_failed: row.assertions_failed,
    ssl_info: safeJsonParseOrPassthrough(row.ssl_info, undefined) as CheckResult['ssl_info'],
...
function parseTransactionCheckRow(row: TransactionCheckRow): TransactionCheck {
  return {
    ...
    steps: safeJsonParseOrPassthrough(row.steps, []) as TransactionCheck['steps'],
    interval: row.interval_seconds,
    enabled: row.enabled,
    created_by: row.created_by,
    created_at: new Date(row.created_at),
...
function parseTransactionResultRow(row: TransactionResultRow): TransactionResult {
  return {
    ...
    step_results: safeJsonParseOrPassthrough(row.step_results, []) as TransactionResult['step_results'],
    checked_at: new Date(row.checked_at),
  };
}
...
function parsePerformanceResultRow(row: PerformanceResultRow): PerformanceResult {
  return {
    ...
    metrics: safeJsonParseOrPassthrough(row.metrics, {}) as PerformanceResult['metrics'],
...
function parseWebhookCheckRow(row: WebhookCheckRow): WebhookCheck {
  return {
    ...
    expected_payload: safeJsonParseOrPassthrough(row.expected_payload, undefined) as WebhookCheck['expected_payload'],
    enabled: row.enabled,
    created_by: row.created_by,
    created_at: new Date(row.created_at),
    updated_at: new Date(row.updated_at),
