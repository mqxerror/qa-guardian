 ...
 *
 * This file contains all CREATE TABLE, CREATE INDEX, and ALTER TABLE statements.
 * Extracted from database.ts for maintainability.
 *
 * Feature #247: Extracted to reduce file size
 *
 ...
 * This includes all table definitions, indexes, and migrations
 */
...
/**
 * Get database index creation SQL
 * Separated for clarity and maintenance
 */
export function getDatabaseIndexSQL(): string {
  return `
...
/**
 * Get database migration SQL for denormalized counts
 * Feature #203 and #230
 */
export function getDatabaseMigrationSQL(): string {
  return `
    ...
    -- Session 155: Add organization_id to test_suites table
    -- The listAllTestSuites and listTestSuitesPaginated queries filter by organization_id
    -- which was missing from the test_suites table, causing 500 errors on /api/v1/stats
    ALTER TABLE test_suites ADD COLUMN IF NOT EXISTS organization_id UUID REFERENCES organizations(id) ON DELETE CASCADE;

    -- Backfill organization_id from the related project
    ...
    CREATE INDEX IF NOT EXISTS idx_test_suites_organization ON test_suites(organization_id);
    CREATE INDEX IF NOT EXISTS idx_test_suites_org_created ON test_suites(organization_id, created_at DESC);
  `;
}

/**
 * Get the complete schema SQL including tables, indexes, and migrations
 */
export function getCompleteSchemaSQL(): string {
  return `
    ${getDatabaseSchemaSQL()}
    ${getDatabaseIndexSQL()}
    ${getDatabaseMigrationSQL()}
  `;
}
