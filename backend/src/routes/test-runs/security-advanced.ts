 ...
 *
 * Feature #245: Types and helpers extracted to security-advanced-types.ts
 */
...
import {
  ...
  secretTypes,
  calculateNextRun,
  getCategoryForRuleId,
  sampleSbomComponents,
  sampleContainerVulnerabilities,
  vulnInfoMap,
  safeVersionsMap,
  packageAlternatives,
} from './security-advanced-types.js';

// Re-export types for backward compatibility
export type { DastScanBody, SecurityReportBody, SecurityPolicyBody, SecurityScanSchedule };
export { securityPolicies, securityScanSchedules };
...
export async function securityAdvancedRoutes(app: FastifyInstance) {
  ...
  app.post<{ Params: { projectId: string }; Querystring: { format?: string; include_dev?: string } }>('/api/v1/security/sbom/:projectId', {
    ...
    const components = sampleSbomComponents;
  ...
  app.get<{
    ...
    // Use vulnerability info from types module
    ...
    if (maxSuggestions >= 2) {
      // Use package alternatives from types module
      const alternative = packageAlternatives[packageName];
      if (alternative) {
        suggestions.push({
  ...
  // ============================================
  // Feature #270: Dependency upgrade recommendations with breaking change risk
  // ============================================
  app.get<{
    Params: { projectId: string };
    Querystring: {
      include_devDependencies?: string;
      min_severity?: string;
      limit?: string;
    }
  }>('/api/v1/projects/:projectId/upgrade-recommendations', {
    preHandler: [authenticate],
  }, async (request, reply) => {
    const { projectId } = request.params;
    const {
      include_devDependencies = 'false',
      min_severity = 'low',
      limit = '20',
    } = request.query;
    const orgId = getOrganizationId(request);
    ...
    const vulnerableDependencies = [
      {
        ...
        severity: 'high',
        vulnerabilities: ['CVE-2021-23337', 'CVE-2020-8203'],
        upgrade_type: 'patch',
        risk_level: 'safe',
        breaking_changes: false,
        changelog_url: 'https://github.com/lodash/lodash/releases/tag/4.17.21',
        description: 'Security fixes for prototype pollution vulnerabilities.',
      },
      {
        package: 'axios',
        ...
        severity: 'high',
        vulnerabilities: ['CVE-2021-3749', 'CVE-2023-45857'],
        upgrade_type: 'major',
        risk_level: 'breaking',
        breaking_changes: true,
        changelog_url: 'https://github.com/axios/axios/releases/tag/v1.6.0',
        description: 'Major version update required. Review breaking changes in request/response handling.',
        migration_notes: [
          'CommonJS require syntax changed',
          'Response data structure modified',
          'Interceptor API updated',
        ],
      },
      {
        package: 'express',
        ...
        risk_level: 'caution',
        breaking_changes: false,
        changelog_url: 'https://github.com/expressjs/express/releases/tag/4.18.2',
        description: 'Minor version with security fixes. Review middleware compatibility.',
      },
      {
        package: 'glob-parent',
        ...
        severity: 'high',
        vulnerabilities: ['CVE-2020-28469'],
        upgrade_type: 'patch',
        risk_level: 'safe',
        breaking_changes: false,
        changelog_url: 'https://github.com/gulpjs/glob-parent/releases/tag/v5.1.2',
        description: 'Patch release fixing ReDoS vulnerability.',
      },
      {
        package: 'minimist',
        ...
        severity: 'critical',
        vulnerabilities: ['CVE-2021-44906', 'CVE-2020-7598'],
        upgrade_type: 'patch',
        risk_level: 'safe',
        breaking_changes: false,
        changelog_url: 'https://github.com/minimistjs/minimist/releases/tag/v1.2.8',
        description: 'Critical prototype pollution fix.',
      },
      {
        package: 'node-fetch',
        ...
        severity: 'medium',
        vulnerabilities: ['CVE-2022-0235'],
        upgrade_type: 'minor',
        risk_level: 'caution',
        breaking_changes: false,
        changelog_url: 'https://github.com/node-fetch/node-fetch/releases/tag/v2.7.0',
        description: 'Minor update fixing cookie handling vulnerability.',
        alternative: {
          package: 'undici',
          reason: 'Native fetch support in Node.js 18+',
        },
      },
    ];
    ...
    const minSeverityLevel = severityOrder[min_severity as keyof typeof severityOrder] ?? 3;
    const filtered = vulnerableDependencies.filter(
      dep => severityOrder[dep.severity as keyof typeof severityOrder] <= minSeverityLevel
    );

    // Limit results
    const limitNum = Math.min(parseInt(limit, 10) || 20, 50);
    const recommendations = filtered.slice(0, limitNum);

    // Calculate summary statistics
    const summary = {
      total_recommendations: recommendations.length,
      by_risk_level: {
        safe: recommendations.filter(r => r.risk_level === 'safe').length,
        caution: recommendations.filter(r => r.risk_level === 'caution').length,
        breaking: recommendations.filter(r => r.risk_level === 'breaking').length,
      },
      by_severity: {
      ...
      total_vulnerabilities: recommendations.reduce((sum, r) => sum + r.vulnerabilities.length, 0),
      actionable_now: recommendations.filter(r => r.risk_level === 'safe').length,
    };
    ...
    return {
      ...
      metadata: {
        generated_at: new Date().toISOString(),
        npm_registry_checked: true,
        include_dev: include_devDependencies === 'true',
        min_severity,
      },
    };
  });
