...
export const generateTest: ToolHandler = async (args, context) => {
  try {
    ...
    // Phase 3D: Track AI failure reason for frontend transparency
    let aiFailureReason: string | undefined;
    // P1a: routing transparency — capture failover details from aiRouter so the
    // frontend can show "Failover: rate limit exceeded" instead of a silent
    // "Generated by anthropic" with no explanation.
  ...
  await expect(page).toHaveTitle(/./);
    ...
    return {
      ...
      ai_metadata: {
        ...
        // Phase 3D: Surface AI failure reason so frontend can display it
        ...(aiFailureReason ? { ai_failure_reason: aiFailureReason } : {}),
        // P1a: routing transparency — show when failover fired and why
