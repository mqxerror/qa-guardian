...
export function DependencyAlertsPage() {
  ...
  const { data: configData, isLoading: isLoadingConfig } = useDependencyAlertConfig();
  const { data: alertsData, isLoading: isLoadingAlerts } = useDependencyAlerts();

  // Mutations
  const updateConfigMutation = useUpdateDependencyAlertConfig();
  const simulateCVEMutation = useSimulateCVE();
  ...
  const handleSimulateCVE = async () => {
    try {
      const result = await simulateCVEMutation.mutateAsync({
        package_name: simulatePackage,
        severity: simulateSeverity,
      });
      toast.success(`Alert created: ${result.alert.cve_id} - Notifications sent: ${result.notifications_sent.join(', ')}`);
  ...
  // Update alert status
  const handleUpdateAlertStatus = async (alertId: string, status: 'acknowledged' | 'dismissed' | 'fixed', dismissedReason?: string) => {
    try {
      await updateAlertStatusMutation.mutateAsync({ alertId, status, dismissed_reason: dismissedReason });
