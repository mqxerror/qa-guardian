...
export function AITestGeneratorPage() {
 ...
 <PageHeader
   ...
   description="Generate Playwright tests from natural-language descriptions. Routes through Kie.ai (primary) with Anthropic fallback."
...
Example: Test that a user can login with valid credentials and see the welcome message on the dashboard"
 ...
 {/* AI Metadata — live provider, model, and routing transparency */}
 {generatedTest.ai_metadata && (
 <div className="flex flex-wrap items-center gap-2 text-xs">
   {/* Provider badge */}
   {(() => {
     const provider = generatedTest.ai_metadata.provider || 'unknown';
     const model = generatedTest.ai_metadata.model;
     const usedRealAi = generatedTest.ai_metadata.used_real_ai;
     const providerLower = String(provider).toLowerCase();
     const isTemplate = !usedRealAi || providerLower === 'template' || providerLower.includes('template');
     const isKie = providerLower === 'kie' || providerLower.includes('kie');
     const isAnthropic = providerLower === 'anthropic' || providerLower.includes('anthropic') || providerLower.includes('claude');
     const badgeClass = isTemplate
     ...
     const icon = isTemplate ? '📋' : isKie ? '⚡' : isAnthropic ? '🧠' : '🤖';
     const label = isTemplate
       ...
       : isKie
         ...
         : isAnthropic
           ...
           : provider;
     return (
       <span className={`px-2 py-1 rounded-md border text-xs font-medium flex items-center gap-1.5 ${badgeClass}`}>
         <span>{icon}</span>
         <span>{label}</span>
   ...
   {/* Fallback-used badge */}
   {generatedTest.ai_metadata.used_fallback && (
     <span className="px-2 py-1 rounded-md border bg-warning/10 text-warning border-warning/30 text-xs flex items-center gap-1">
       <span>⚠</span>
       <span>Failover</span>
       {generatedTest.ai_metadata.fallback_reason && (
         <span className="opacity-70">({generatedTest.ai_metadata.fallback_reason})</span>
       )}
     </span>
   )}
   {/* Latency */}
   {generatedTest.ai_metadata.latency_ms !== undefined && (
     <span className="text-muted-foreground">
       {generatedTest.ai_metadata.latency_ms}ms
     </span>
   )}
   {/* Token counts */}
   {generatedTest.ai_metadata.tokens_used && (
     <span className="text-muted-foreground">
       {generatedTest.ai_metadata.tokens_used.input}/{generatedTest.ai_metadata.tokens_used.output} tokens
     </span>
   )}
 </div>
 )}
