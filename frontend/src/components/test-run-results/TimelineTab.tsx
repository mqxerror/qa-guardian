...
import { Download, Loader2, Play, ChevronDown, AlertTriangle, X } from 'lucide-react';
...
export default function TimelineTab({
 ...
 step.status === 'passed' ? 'border-success/20' :
 ...
 <span className="px-1.5 py-0.5 text-xs bg-primary/10 text-primary rounded" title={`${step.stepNetworkRequests.length} network requests`}>
 ...
 <span className="inline-flex items-center gap-1 px-2 py-1 rounded bg-warning/10 text-warning text-xs">
 ...
 <div className="col-span-2">
 <span className="inline-flex items-center gap-1 px-2 py-1 rounded bg-destructive/10 text-destructive text-xs">
 ...
 (metric.value || 0) >= 90 ? 'text-success' :
 (metric.value || 0) >= 50 ? 'text-warning' :
 'text-destructive'
 }`}>
 {metric.value}
 </div>
 ...
 (step.load_test.error_rate ?? 0) > 5 ? 'bg-destructive/10' :
 (step.load_test.error_rate ?? 0) > 1 ? 'bg-warning/10' :
 'bg-success/10'
 }`}>
 <div className={`text-xl font-bold ${
 (step.load_test.error_rate ?? 0) > 5 ? 'text-destructive' :
 (step.load_test.error_rate ?? 0) > 1 ? 'text-warning' :
 'text-success'
 }`}>
 ...
 req.status >= 400 ? 'text-destructive' : 'text-warning'
 }`}>
 {req.status || '-'}
 </span>
 ...
 req.status >= 400 ? 'bg-destructive/10 text-destructive' : 'bg-warning/10 text-warning'
 }`}>
 {req.method}
 </span>
