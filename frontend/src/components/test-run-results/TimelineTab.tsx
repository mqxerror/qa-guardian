...
export default function TimelineTab({
 ...
 <span className="px-1.5 py-0.5 text-xs bg-blue-100 text-blue-700 rounded" title={`${step.stepNetworkRequests.length} network requests`}>
 ...
 (step.load_test.error_rate ?? 0) > 5 ? 'bg-red-100' :
 (step.load_test.error_rate ?? 0) > 1 ? 'bg-yellow-100' :
 'bg-green-100'
 }`}>
