...
export const LighthouseResultCard: React.FC<LighthouseResultCardProps> = ({
  ...
  return (
    <div className={`bg-card border rounded-xl overflow-hidden shadow-lg shadow-black/5 dark:shadow-black/20 ${
      lighthouseStatus === 'excellent' ? 'border-green-300 dark:border-green-700' :
      lighthouseStatus === 'needs-improvement' ? 'border-yellow-300 dark:border-yellow-700' :
      ...
      <div className={`border-b-4 ${
        ...
        <div className={`p-5 bg-gradient-to-r ${
          lighthouseStatus === 'excellent' ? 'from-green-50 to-emerald-50/50 dark:from-green-900/30 dark:to-emerald-900/10' :
          lighthouseStatus === 'needs-improvement' ? 'from-yellow-50 to-amber-50/50 dark:from-yellow-900/30 dark:to-amber-900/10' :
          'from-red-50 to-rose-50/50 dark:from-red-900/30 dark:to-rose-900/10'
        }`}>
          {/* Header row with test name and actions */}
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              <span className="px-3 py-1 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 text-xs font-medium rounded-full flex items-center gap-1.5">
          ...
          <div className="flex flex-wrap items-center gap-2 mb-4">
            <div className={`px-4 py-1.5 rounded-full text-sm font-semibold flex items-center gap-2 ${
              lighthouseStatus === 'excellent' ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300' :
              lighthouseStatus === 'needs-improvement' ? 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300' :
          ...
          {topOpportunities.length > 0 && lighthouseStatus !== 'excellent' && (
            <div className="bg-white/50 dark:bg-black/20 rounded-lg p-3 border border-current/10">
